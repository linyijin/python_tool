...
function [ ] = spin45_Norm( othername, img, mask, angle, imgsavepath, datapathmask ,slidwindow, overlapping, Markthreshold, txtpath, possamplesavepath, postxtpath)
%SPIN45 此处显示有关此函数的摘要
%   此处显示详细说明
...
%                     imwrite(win, cut_possamplemasksavename);
                ...
                elseif x > Markthreshold
                    ...
                    areaInfo_10 = find(win == 10);
                    [Info_10_x, Info_10_y] = size(areaInfo_10);   
					areaInfo_11 = find(win == 11);
                    [Info_11_x, Info_11_y] = size(areaInfo_11);  
                    if Info_1_x > 0
                    ...
                    if Info_10_x > 0
                        savename = strcat(savename,'-', 'chousha');
                    end
                    if Info_11_x > 0
                        savename = strcat(savename,'-', 'sizheyin');
                    end
                    cut_name = strcat(savename, '.jpg');
                    mask_cut_name = strcat(savename, '.png');
                    cut_imgsavename = strcat(imgsavepath,cut_name);
